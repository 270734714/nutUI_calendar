<template>
  <!--<div>{{defaultValueData}}</div>-->
  <!--<div>{{defaultValueDataE}}</div>-->
  <nut-picker
    :is-visible="isVisible"
    :title="title"
    :list-data="listData"
    :default-value-data="defaultValueData"
    :default-value-data-e="defaultValueDataE"
...
export default {
  ...
  methods: {
    ...
    resetDefaultValue() {
      ...
      let cacheDefaultValueE = null;
      if (!this.defaultValueE || !Utils.isDateString(this.defaultValueE)) {
        switch (this.type) {
          case 'time':
            cacheDefaultValueE = this.isSetSecond ? `00:00:00` : `00:00`;
            break;
          case 'date':
          case 'datetime':
            cacheDefaultValueE = `${this.startDateArrE[0]}-${this.startDateArrE[1]}-${this.startDateArrE[2]} ${this.startDateArrE[3]}:${this.startDateArrE[4]}`;
            break;
        }
      } else {
        cacheDefaultValueE = this.defaultValueE;
      }

      let splitArr = cacheDefaultValue.split(' ');
      let splitArrE = cacheDefaultValueE.split(' ');
      if (this.type === 'time') {
        ...
        let timeArrE = splitArrE[0].split(':');
        this.isUse12Hours && timeArrE.push(this.isAm ? this.use12Hours[0] : this.use12Hours[1]);
        this.cacheDefaultDataE = this.getCacheData(timeArrE);
      } else {
        let cacheData = [...splitArr[0].replace(/-/g, '/').split('/')];
        let cacheDataE = [...splitArrE[0].replace(/-/g, '/').split('/')];
        if (this.type == 'datetime') {
          cacheData = [...cacheData, ...splitArr[1].split(':')];
          cacheDataE = [...cacheDataE, ...splitArrE[1].split(':')];
        }
        ...
        this.cacheDefaultDataE = this.getCacheData(cacheDataE);
//        this.updateYear = this.cacheDefaultData[0];
//        this.updateMonth = this.cacheDefaultData[1];
//        this.updateDay = this.cacheDefaultData[2];
//        this.updateHour = this.cacheDefaultData[3];
      ...
      this.defaultValueDataE = [...this.cacheDefaultDataE];
    },
